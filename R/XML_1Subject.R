...
#######NAL-N2 target curves 
f <- c(250,500,1000,1500,2000,3000,4000,8000)

nal <- list()
bla <- xmltop[['Session']][['Settings']][['Settings']][['HearingThresholdLevelInSPLLeft']]

# left 
nal$left <- 0
for (l in 1:(length(f)-1)) {
  nal$left <- c(nal$left,as.numeric(xmlValue(bla[[l]][['Y']])))

}

bla <- xmltop[['Session']][['Settings']][['Settings']][['HearingThresholdLevelInSPLRight']]

# right
nal$right <- 0
for (l in 1:(length(f)-1)) {
  nal$right <- c(nal$right,as.numeric(xmlValue(bla[[l]][['Y']])))
  
}


NAL <- as.data.frame(nal)
NAL <- cbind(f,NAL)
